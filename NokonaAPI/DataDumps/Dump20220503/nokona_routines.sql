...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `jobview` AS select `jh`.`JobId` AS `JobID`,trim(trim(trailing '-LH' from trim(trailing '-RH' from `jh`.`JobId`))) AS `JobIdTrimmed`,`jd`.`OpCode` AS `OpCode`,`jd`.`Sequence` AS `Sequence` from (`jobheader` `jh` left join `jobdetail` `jd` on((trim(trim(trailing '-LH' from trim(trailing '-RH' from `jh`.`JobId`))) = `jd`.`JobId`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2022-05-03 15:54:46
