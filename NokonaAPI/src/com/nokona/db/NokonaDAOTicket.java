...
public class NokonaDAOTicket extends NokonaDAO  implements NokonaDatabaseTicket {
	...
	PreparedStatement psGetTicketByKey;
	PreparedStatement psGetTicketHeaderByKey;
	PreparedStatement psGetTicketHeaders;
	PreparedStatement psGetEmployeeByEmpId;
	PreparedStatement psGetTickets;
	PreparedStatement psGetTicketsByModel;
	...
	public List<Ticket> getTickets() throws DatabaseException {
		...
		if (psGetTickets == null) {
			try {
				psGetTickets = getConn().prepareStatement("Select * from ticketheader join ticketdetail on ticketheader.key = ticketdetail.key " + 
	...
	public Ticket getTicketByKey(long key) throws DatabaseException {
		...
		if (psGetTicketByKey == null) {
			try {
				psGetTicketByKey = conn.prepareStatement("Select * from ticketheader join ticketdetail on ticketheader.key = ticketdetail.key where " +  
...
public List<Ticket> getTicketsByModel(String model) throws DatabaseException {
	...
	if (psGetTicketsByModel == null) {
		try {
			psGetTicketsByModel = getConn().prepareStatement("Select * from ticketheader join ticketdetail on ticketheader.key = ticketdetail.key " + 
...
@Override
public Ticket addTicket(Ticket ticket) throws DatabaseException {
...
public List<TicketHeader> getTicketHeaders() throws DatabaseException {
	...
	if (psGetTicketHeaders == null) {
		try {
			psGetTicketHeaders = conn.prepareStatement("Select * from ticketheader " +  
					"order by model, dateCreated, Status");
...
public TicketHeader getTicketHeaderByKey(long headerKey) throws DatabaseException {
	...
	if (psGetTicketHeaderByKey == null) {
		try {
			psGetTicketHeaderByKey = conn.prepareStatement("Select * from ticketheader where " +  
					"ticketheader.key = ?");
